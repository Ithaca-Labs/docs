---
title: "Self-host openx402"
description: "Deploy and operate the facilitator, PostgreSQL, search, and optional MCP server."
...
production-ready. See [Security](/operations/security#pubnet) before changing that.

<Warning>
  Do not enable pubnet until every startup gate, audit, fee calibration, key-management, TTL, backup, and incident-response requirement is satisfied.
</Warning>
...
Choose a profile before adding provider keys:

| Profile | Embeddings | Reranking | Use it when |
| --- | --- | --- | --- |
| Testnet default | Disabled or lexical fallback | Disabled | You need a zero-key local smoke test. |
| Hosted Railway | Remote OpenAI-compatible provider | Disabled | You want managed search and only supply `OPENROUTER_API_KEY`. |
| Lightweight self-host | Disabled | Disabled | You want the smallest deployment and deterministic PostgreSQL FTS. |
| Local semantic | Local optional runtime | Disabled by default | You can package model weights and accept the image/storage cost. |

The profile changes performance and search quality only. It cannot disable
signature validation, enforcing simulation, fee gates, idempotency, channel
fencing, or the pubnet startup gates. See the complete [configuration
reference](/reference/configuration) for every supported knob and its default.
...
[facilitator configuration reference](https://github.com/Ithaca-Labs/openx402/blob/main/facilitator/docs/CONFIGURATION.md).
...
See [Search and indexing](https://github.com/Ithaca-Labs/openx402/blob/main/facilitator/docs/SEARCH.md) for provider contracts,
...
in the [MCP server guide](https://github.com/Ithaca-Labs/openx402/blob/main/mcp-server/README.md).
...
Use the complete [Railway deployment guide](https://github.com/Ithaca-Labs/openx402/blob/main/deploy/railway/README.md) for
...
- [Upto deployment boundary](/guides/upto)
